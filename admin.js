...
document.getElementById('login-form').onsubmit = async (e) => {
    ...
    try {
        ...
        if (response && response.sessionToken) {
            sessionStorage.setItem('hasleagueAdminAuth', response.sessionToken);
            showAdminControls();
            loadTeamOptions();
            loadMatchOptions();
            alert('Giriş başarılı. Admin paneline erişebilirsiniz.');
        } else {
            throw new Error('Geri dönüş token\'ı alınamadı.');
        }
    } catch (error) {
...
const adminToken = sessionStorage.getItem('hasleagueAdminAuth');
if (adminToken && typeof adminToken === 'string' && adminToken.startsWith('token_')) {
    showAdminControls();
    loadTeamOptions();
    loadMatchOptions();
} else {
    sessionStorage.removeItem('hasleagueAdminAuth');
    showLogin();
}
